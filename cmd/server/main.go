...
func main() {
	...
	// GET /v1/repos/{repo_id}/scans - Get recent scan history
	mux.HandleFunc("GET /v1/repos/{repo_id}/scans", func(w http.ResponseWriter, r *http.Request) {
		repoIDStr := r.PathValue("repo_id")
		repoID, err := uuid.Parse(repoIDStr)
		if err != nil {
			http.Error(w, "Invalid repo ID", http.StatusBadRequest)
		...
		scans, err := store.GetScans(context.Background(), repoID, 10)
		if err != nil {
			log.Printf("Error fetching scans: %v", err)
	...
	// GET /v1/repos/{repo_id}/findings - Get currently open findings table
	mux.HandleFunc("GET /v1/repos/{repo_id}/findings", func(w http.ResponseWriter, r *http.Request) {
		repoIDStr := r.PathValue("repo_id")
		repoID, err := uuid.Parse(repoIDStr)
		if err != nil {
			http.Error(w, "Invalid repo ID", http.StatusBadRequest)
		...
		findings, err := store.GetOpenFindings(context.Background(), repoID)
		if err != nil {
